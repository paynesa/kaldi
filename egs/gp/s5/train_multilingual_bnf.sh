...
# e.g. decode_lang_list=(FR)
decode_lang_list=(FR GE)
...
bnf_dim=40           # If non-empty, the bottleneck layer with this dimension is added at two layers before softmax.
dim=625
...
if [ $stage -le 14 ]; then
    ...
    steps/nnet3/make_bottleneck_features.sh \
        --use-gpu true --nj 70 --cmd "$train_cmd" \
